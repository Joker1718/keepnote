...
class Extension(keepnote.gui.extension.Extension):
    def __init__(self, app):
        ...
        self.commands = [
            ...
            AppCommand(
                "minimize", self.on_minimize_windows, help="minimize all windows"
            ),
            AppCommand(
                "toggle-windows", self.on_toggle_windows, help="toggle all windows"
            ),
            # extension commands
            AppCommand(
                "install",
                self.on_install_extension,
                metavar="FILENAME",
                help="install a new extension",
            ),
            AppCommand(
                "uninstall",
                self.on_uninstall_extension,
                metavar="EXTENSION_NAME",
                help="uninstall an extension",
            ),
            AppCommand(
                "tmp_ext",
                self.on_temp_extension,
                metavar="FILENAME",
                help="add an extension just for this session",
            ),
            AppCommand(
                "ext_path",
                self.on_extension_path,
                metavar="PATH",
                help="add an extension path for this session",
            ),
            AppCommand(
                "quit",
                lambda app, args: gobject.idle_add(app.quit),
                help="close all KeepNote windows",
            ),
            # notebook commands
            AppCommand(
                "view", self.on_view_note, metavar="NOTE_URL", help="view a note"
            ),
            AppCommand(
                "new", self.on_new_note, metavar="PARENT_URL", help="add a new note"
            ),
            AppCommand(
                "search-titles",
                self.on_search_titles,
                metavar="TEXT",
                help="search notes by title",
            ),
            AppCommand(
                "upgrade",
                self.on_upgrade_notebook,
                metavar="[v VERSION] NOTEBOOK...",
                help="upgrade a notebook",
            ),
            # misc
            AppCommand(
                "screenshot", self.on_screenshot, help="insert a new screenshot"
            ),
        ]
    ...
    def on_search_titles(self, app, args):
        ...
        for nodeid, title in nodes:
            sys.stdout.write(f"{title}\t{keepnote.notebook.get_node_url(nodeid)}\n")
    ...
    def on_upgrade_notebook(self, app, args):
        ...
        for filename in files:
            keepnote.log_message(
                "upgrading notebook to version %d: %s\n" % (version, filename)
            )
