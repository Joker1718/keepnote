...
def get_valid_filename(filename, default="folder", maxlen=MAX_LEN_NODE_FILENAME):
    """
...
class NoteBookConnectionFS(NoteBookConnection):
    ...
    def _move_to_lostdir(self, filename):
        ...
        keepnote.log_message(
            f"moving data to lostdir '{filename}' => '{new_filename}'\n"
        )
        try:
            os.rename(filename, new_filename)
        except OSError as e:
            raise ConnectionError(f"unable to store lost file '{filename}'", e)
    ...
    def create_node(self, nodeid, attr, _path=None, _root=False):
        ...
        try:
            os.mkdir(path)
            self._write_attr(
                self._get_node_attr_file(nodeid, path), attr, self._attr_defs
            )
        ...
        if not self._index:
            ...
            self._index = notebook_index.NoteBookIndex(self, self._get_index_file())
        self._index.add_node(
            nodeid, parentid, basename, attr, mtime=get_path_mtime(path)
        )
    ...
    def update_node(self, nodeid, attr):
        ...
        # write attrs
        path = self._get_node_path(nodeid)
        self._write_attr(get_node_meta_file(path), attr, self._attr_defs)

        if parentid != parentid2:
            # move to a new parent
            self._rename_node_dir(nodeid, attr, parentid, parentid2, path)
        elif parentid and title_index and title_index != attr.get("title", ""):
            ...
            self._index.add_node(
                nodeid, parentid2, basename, attr, mtime=get_path_mtime(path)
            )
    ...
    def _rename_node_dir(self, nodeid, attr, parentid, new_parentid, path):
        ...
        try:
            ...
            raise keepnote.compat.notebook_v4.NoteBookError(
                _(f"Cannot rename '{path}' to '{new_path}'"), e
            )
        ...
        self._index.add_node(
            nodeid, new_parentid, basename, attr, mtime=get_path_mtime(new_path)
        )

        # update parent too
        self._index.set_node_mtime(parentid, get_path_mtime(os.path.dirname(path)))
        if new_parentid != parentid:
            self._index.set_node_mtime(new_parentid, get_path_mtime(new_parent_path))
    ...
    def _list_children_attr(self, nodeid, _path=None, _full=True):
        ...
        try:
            ...
            raise keepnote.compat.notebook_v4.NoteBookError(
                _("Do not have permission to read folder contents: %s") % path, e
            )
        ...
        for filename in files:
            ...
            if os.path.exists(get_node_meta_file(path2)):
                try:
                    ...
                    keepnote.log_error(f"error reading {path2}")
                    continue
    ...
    def _read_node(self, parentid, path, _full=True, _force_index=False):
        ...
        if _force_index:
            # reindex this node
            self._index.add_node(nodeid, parentid, basename, attr, get_path_mtime(path))
        else:
    ...
    def _reindex_node(self, nodeid, parentid, path, attr, mtime, warn=True):
        ...
        if warn:
            keepnote.log_message(f"Unmanaged change detected. Reindexing '{path}'\n")
        ...
        for path2 in iter_child_node_paths(path):
            attr2 = self._read_node(nodeid, path2, _full=False, _force_index=True)
            # self._index.add_node(
            #    attr2["nodeid"], nodeid,
        ...
        # reindex this node
        self._index.add_node(nodeid, parentid, os.path.basename(path), attr, mtime)
        ...
        try:
            ...
            for key, val in attr.iteritems():
                ...
                if attr_def is not None:
                    out.write(
                        f'<attr key="{key}">{escape(attr_def.write(val))}</attr>\n'
                    )
                elif key == "version":
                    # skip version attr
                    pass
                    ...
                    out.write(f'<attr key="{key}">{escape(val.value)}</attr>\n')
                else:
                    # drop attribute
                    pass
    ...
    def _read_attr(self, filename, attr_defs, recover=True):
        ...
        try:
            ...
            raise keepnote.compat.notebook_v4.NoteBookError(
                _(f"Error reading meta data file '{filename}'"), e
            )
    ...
    def _recover_attr(self, filename):
        ...
        try:
            ...
            keepnote.log_error(f"failed to recover '{filename}'")
            pass
    ...
    def read_data_as_plain_text(self, nodeid):
        ...
        try:
            ...
            yield from keepnote.compat.notebook_v4.read_data_as_plain_text(infile)
            infile.close()
        except:
            pass
    ...
    def list_files(self, nodeid, filename="", _path=None):
        ...
        for filename in filenames:
            if filename != NODE_META_FILE and not filename.startswith("__"):
    ...
    def index_attr(self, key, index_value=False):
        ...
        if issubclass(datatype, basestring):
            ...
            raise Exception(f"unknown attr datatype '{repr(datatype)}'")

        self._index.add_attr(
            notebook_index.AttrIndex(key, index_type, index_value=index_value)
        )
