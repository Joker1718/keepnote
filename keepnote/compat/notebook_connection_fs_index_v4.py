...
class AttrIndex:
    ...
    def init(self, cur):
        ...
        cur.execute(f"""CREATE TABLE IF NOT EXISTS {self._table_name}
                        ...
                        """)
        cur.execute(f"""CREATE INDEX IF NOT EXISTS {self._index_name}
                           ON {self._table_name} (nodeid);""")

        if self._index_value:
            cur.execute(f"""CREATE INDEX IF NOT EXISTS {self._index_value_name}
                           ON {self._table_name} (value);""")
    ...
    def remove_node(self, cur, nodeid):
        ...
        cur.execute(f"DELETE FROM {self._table_name} WHERE nodeid=?", (nodeid,))

    def get(self, cur, nodeid):
        """Get information for a node from the index"""
        cur.execute(
            f"""SELECT value FROM {self._table_name} WHERE nodeid = ?""", (nodeid,)
        )
    ...
    def set(self, cur, nodeid, value):
        ...
        # insert new row
        cur.execute(
            f"""INSERT INTO {self._table_name} VALUES (?, ?)""", (nodeid, value)
        )
...
class NoteBookIndex:
    ...
    def _set_version(self, version=INDEX_VERSION):
        ...
        self.con.execute("INSERT INTO Version VALUES (?, datetime('now'));", (version,))
    ...
    def init_index(self, auto_clear=True):
        ...
        try:
            ...
            try:
                ...
                if not list(
                    con.execute("""SELECT 1 FROM sqlite_master 
                                   WHERE name == 'fulltext';""")
                ):
            ...
            keepnote.log_message(f"reinitializing index '{self._index_file}'\n")
            self.clear()
    ...
    def check_index(self):
        ...
        keepnote.log_message(f"{time.time() - start:f} seconds\n")
    ...
    def index_all(self, rootid=None):
        ...
        def preorder(conn, nodeid):
            ...
            while len(queue) > 0:
                nodeid = queue.pop()
                yield nodeid
                queue.extend(conn._list_children_nodeids(nodeid, _index=False))
    ...
    def set_node_mtime(self, nodeid, mtime=None):
        ...
        self.con.execute(
            """UPDATE NodeGraph SET mtime = ? WHERE nodeid = ?;""", (mtime, nodeid)
        )
    ...
    def add_node(self, nodeid, parentid, basename, attr, mtime):
        ...
        try:
            ...
            keepnote.log_error(
                f"error index node {nodeid} '{attr.get('title', '')}'"
            )
            self._on_corrupt(e, sys.exc_info()[2])
    ...
    def insert_text(self, nodeid, text):
        ...
        if list(self.cur.execute("SELECT 1 FROM fulltext WHERE nodeid = ?", (nodeid,))):
            self.cur.execute(
                "UPDATE fulltext SET content = ? WHERE nodeid = ?;", (text, nodeid)
            )
        else:
            self.cur.execute("INSERT INTO fulltext VALUES (?, ?);", (nodeid, text))
    ...
    def get_node_path(self, nodeid):
        ...
        try:
            while parentid != self._uniroot:
                ...
                self.cur.execute(
                    """SELECT nodeid, parentid, basename
                                FROM NodeGraph
                                WHERE nodeid=?""",
                    (nodeid,),
                )
    ...
    def get_node(self, nodeid):
        ...
        try:
            self.cur.execute(
                """SELECT nodeid, parentid, basename, mtime
                                FROM NodeGraph
                                WHERE nodeid=?""",
                (nodeid,),
            )
    ...
    def search_titles(self, query):
        ...
        try:
            ...
            table_name = self._attrs["title"].get_table_name()
            self.cur.execute(
                f"""SELECT nodeid, value FROM {table_name} WHERE value LIKE ?
    ...
    def _search_manual(self, words):
        ...
        while len(stack) > 0:
            ...
            infile = chain([title], self._nconn.read_data_as_plain_text(nodeid))
