...
def write_node_tree(out, conn, nodeid=None):
    ...
    if attr.get("content_type", "") == "text/xhtml+xml":
        ...
        url = format_node_path("", nodeid, attr["payload_filename"])
    else:
        url = format_node_path("", nodeid, "")
    title = attr.get("title", "page")
    out.write(f"<a href='{url}'>{title}</a>")
    out.write("<ul>")
...
class BaseNoteBookHttpServer:
    def __init__(self, conn, host="", port=8000):
        ...
        self.app.get("/static/<filename:re:.*>", callback=self.static_file_view)

        # Notebook node routes.
        self.app.post("/notebook/", callback=self.command_view)
        self.app.get("/notebook/nodes/", callback=self.read_root_view)
        self.app.get("/notebook/nodes/<nodeid:re:[^/]+>", callback=self.read_node_view)
        self.app.post("/notebook/nodes/", callback=self.create_node_view)
        self.app.post(
            "/notebook/nodes/<nodeid:re:[^/]+>", callback=self.create_node_view
        )
        self.app.put(
            "/notebook/nodes/<nodeid:re:[^/]+>", callback=self.update_node_view
        )
        self.app.delete(
            "/notebook/nodes/<nodeid:re:[^/]+>", callback=self.delete_node_view
        )
        self.app.route(
            "/notebook/nodes/<nodeid:re:[^/]+>", "HEAD", callback=self.has_node_view
        )

        # Notebook file routes.
        self.app.get(
            "/notebook/nodes/<nodeid:re:[^/]*>/<filename:re:.*>",
    ...
    def serve_forever(self, debug=False):
        ...
        self.server = bottle.WSGIRefServer(host=self.host, port=self.port, debug=debug)
        self.app.run(
            host=self.host,
            port=self.port,
            server=self.server,
